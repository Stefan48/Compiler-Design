...
public class ASTPrintVisitor implements ASTVisitor<String> {
    ...
    public String visit(CallNode callNode) {
        String output = addIndent("implicit dispatch\n");
        indent++;
        output += addIndent(callNode.token.getText() + "\n");
    ...
    @Override
    public String visit(DispatchNode dispatchNode) {
        String output = addIndent(".\n");
        indent++;
        output += dispatchNode.caller.accept(this);
        if (dispatchNode.type != null) {
            output += addIndent(dispatchNode.type.getText() + "\n");
        }
        output += addIndent(dispatchNode.func.getText() + "\n");
        for (ExpressionNode param : dispatchNode.params) {
            output += param.accept(this);
        }
        indent--;
        return output;
    ...
    @Override
    public String visit(DefinitionNode definitionNode) {
        String output = addIndent("formal\n");
        indent++;
        output += addIndent(definitionNode.name.getText() + "\n");
        output += addIndent(definitionNode.type.getText() + "\n");
        indent--;
        return output;
    }
    ...
    public String visit(FuncDefNode funcDefNode) {
        ...
        for (DefinitionNode param : funcDefNode.params) {
            output += param.accept(this);
        }
